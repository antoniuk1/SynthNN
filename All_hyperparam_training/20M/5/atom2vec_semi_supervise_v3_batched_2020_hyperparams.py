...
file_output = open("semi_weights_testing_pos_20M" + model_name_params +  ".txt","a")
...
file_output = open("semi_weights_testing_neg_20M" + model_name_params +  ".txt","a")
...
for i in range(num_steps):   
    ...
    if(i%1000==0):
        ...
        np.savetxt('performance_matrix_TL_v3_20M_' + model_name_params+ '.txt',performance_array, fmt='%s')

    if(i%1000==0):
        if(val_accuracy>best_perf):
            model_name='20M_synth_v3_semi' + model_name_params + '.txt'
            best_perf=val_accuracy
            W1_val=sess.run(W1)
            F1_val=sess.run(F1)
            F2_val=sess.run(F2)
