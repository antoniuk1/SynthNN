...
def get_batch(batch_size, neg_positive_ratio, use_semi_weights, model_name, seed=False, seed_value=0):
    ...
    if(use_semi_weights):
        weights1=[]
        file=open('semi_weights_testing_pos_6M' + model_name + '.txt','r')
        ...
        weights0=[]
        file=open('semi_weights_testing_neg_6M' + model_name + '.txt','r')
        content=file.readlines()
        for i in idxs0:
...
file_output = open("semi_weights_testing_pos_6M" + model_name_params +  ".txt","a")
...
file_output = open("semi_weights_testing_neg_6M" + model_name_params +  ".txt","a")
...
for i in range(num_steps):   
    ...
    if(i%1000==0):
        ...
        np.savetxt('performance_matrix_TL_v3_6M_' + model_name_params+ '.txt',performance_array, fmt='%s')

    if(i%1000==0):
        if(val_accuracy>best_perf):
            model_name='6M_synth_v3_semi' + model_name_params + '.txt'
            best_perf=val_accuracy
            W1_val=sess.run(W1)
            F1_val=sess.run(F1)
            F2_val=sess.run(F2)
