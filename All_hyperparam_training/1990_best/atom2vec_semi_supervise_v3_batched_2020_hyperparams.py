...
def get_batch_val(neg_positive_ratio):
    ...
    data1=[]
    f=open('icsd_full_data_unique_no_frac_1990_nopenta.txt')
    i=0
    for line in f:
...
def get_batch(batch_size, neg_positive_ratio, use_semi_weights, model_name, seed=False, seed_value=0):
    ...
    #only sample from first 90% of dataset ( need to shuffle first because GNN_icsd is alphabetical!)
    data1=[]
    pulled_lines1,idxs1=random_lines('icsd_full_data_unique_no_frac_1990_nopenta.txt', noTr_positives,num_positive_examples)
...
file_positives=open('icsd_full_data_unique_no_frac_1990_nopenta.txt', 'r')
